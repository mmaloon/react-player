...
test('end - parses seconds', t => {
  t.is(parseEndTime(YOUTUBE_URL + '?end=162'), 162)
})

test('end - parses stamps', t => {
...
test('end - parses with other params', t => {
  t.is(parseEndTime(YOUTUBE_URL + '?param=1&end=32'), 32)
})

test('end - parses using a hash', t => {
  t.is(parseEndTime(YOUTUBE_URL + '#end=32'), 32)
})

test('end - returns undefined for invalid stamps', t => {
