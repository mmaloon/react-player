...
test('start - parses seconds', t => {
  t.is(parseStartTime(YOUTUBE_URL + '?start=162'), 162)
})

test('start - parses stamps', t => {
...
test('start - parses with other params', t => {
  t.is(parseStartTime(YOUTUBE_URL + '?param=1&start=32'), 32)
})

test('start - parses using t', t => {
  t.is(parseStartTime(YOUTUBE_URL + '?t=32'), 32)
})

test('start - parses using a hash', t => {
  t.is(parseStartTime(YOUTUBE_URL + '#t=32'), 32)
  t.is(parseStartTime(YOUTUBE_URL + '#start=32'), 32)
})

test('start - returns undefined for invalid stamps', t => {
